class Solution:
    ...
    def my_method(
        ...
        if self.temp_dict.get(key) is not None:
            return False
        if (
            jug1_status == targetCapacity
            or jug2_status == targetCapacity
            or jug1_status + jug2_status == targetCapacity
        ):
            return True

        self.temp_dict[key] = 1
        temp_result = self.my_method(
            jug1Capacity, jug2Capacity, 0, jug2_status, targetCapacity
        )
        if temp_result:
            return True
        temp_result = self.my_method(
            jug1Capacity, jug2Capacity, jug1Capacity, jug2_status, targetCapacity
        )
        if temp_result:
            return True
        temp_result = self.my_method(
            jug1Capacity, jug2Capacity, jug1_status, 0, targetCapacity
        )
        if temp_result:
            return True
        temp_result = self.my_method(
            jug1Capacity, jug2Capacity, jug1_status, jug2Capacity, targetCapacity
        )
        if temp_result:
            return True
